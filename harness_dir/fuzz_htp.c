/**
 * @file: AFL Persistent Mode Harness
 * @author: echel0n_1881
 * compilation steps:
CC=afl-cc CXX=afl-c++ AR=llvm-ar RANLIB=llvm-ranlib AS=llvm-as LD=afl-ld-lto
CFLAGS=-DHTP_DEBUG ./configure CC=afl-cc CXX=afl-c++ AR=llvm-ar
RANLIB=llvm-ranlib AS=llvm-as LD=afl-ld-lto CFLAGS=-DHTP_DEBUG * afl-cc -flto
-O0 fuzz_afl.c -I../ -I../../ -I../../htp/ -I../../test
../../htp/.libs/libhtp.so.2.0.0 ../test.c -o harness_htp * CC=afl-cc CXX=afl-c++
AR=llvm-ar RANLIB=llvm-ranlib AS=llvm-as LD=afl-ld-lto make -j24 all
afl-cc -flto -O0 fuzz_afl.c -I../ -I../../ -I../../htp/ -I../../test
../../htp/.libs/libhtp.so.2.0.0 ../test.c -o harness_htp
...
static int HTPCallbackRequestHeaderData(htp_tx_data_t *tx_data) {
  ...
  if (tx_data->len > 0) {
    fprintf(logfile, "HTPCallbackRequestHeaderData %x %x\n", tx_data->data[0],
            tx_data->data[(uintmax_t)tx_data->len - 1]);
  }
  return 0;
}
...
int main() {
  ...
  htp_connp_open(connp, (const char *)"192.168.2.3", 12345,
                 (const char *)"192.168.2.2", 80, NULL);
  ...
  while (__AFL_LOOP(100000)) {
    ...
    if (test.chunk_direction == CLIENT) {
      ...
      if (out_data_other) {
        if (out_data == NULL) {
          rc = htp_connp_res_data(connp, NULL, NULL,
                                  out_data_len - out_data_offset);
        } else {
          rc = htp_connp_res_data(connp, NULL, out_data + out_data_offset,
                                  out_data_len - out_data_offset);
        }
        if (rc == HTP_STREAM_ERROR) {
          break;
      ...
      if (in_data_other) {
        if (in_data == NULL) {
          rc = htp_connp_req_data(connp, NULL, NULL,
                                  in_data_len - in_data_offset);
        } else {
          rc = htp_connp_req_data(connp, NULL, in_data + in_data_offset,
                                  in_data_len - in_data_offset);
        }
        if (rc == HTP_STREAM_ERROR) {
          break;
  ...
  if (out_data_other) {
    if (out_data == NULL) {
      (void)htp_connp_res_data(connp, NULL, NULL,
                               out_data_len - out_data_offset);
    } else {
      (void)htp_connp_res_data(connp, NULL, out_data + out_data_offset,
                               out_data_len - out_data_offset);
    }
  }
